...
def format_comparison_table(results: list[dict]) -> str:
    ...
    lines = [header, sep]
    for r in results:
        prompt_short = r.get("prompt", "")[:40]
        gpt4o_short = r.get("gpt4o_response", "")[:40]
        mini_short = r.get("mini_response", "")[:40]
        gpt4o_lat = f"{r.get('gpt4o_latency', 0):.3f}s"
        mini_lat = f"{r.get('mini_latency', 0):.3f}s"
        lines.append(f"{prompt_short:<42} | {gpt4o_short:<42} | {mini_short:<42} | {gpt4o_lat:>14} | {mini_lat:>12}")
    return "\n".join(lines)
